const model = require("../models/cuisines");
 ...
 */
const nameExistsExcludingItself = async(id, name) => {
    return new Promise((resolve, reject) => {
        model.findOne({
            ...
            (err, item) => {
                utils.itemAlreadyExists(err, item, reject, "NAME_ALREADY_EXISTS");
                resolve(false);
            }
        );
    });
...
exports.updateItem = async(req, res) => {
    try {
        req = matchedData(req);
        const id = await utils.isIDGood(req.id);
        const doesSignExists = await nameExistsExcludingItself(id, req.name);
        if (!doesSignExists) {
...
exports.updateCusioneImageItem = async(req, res) => {
    try {
        req = matchedData(req);
        const id = await utils.isIDGood(req.id);
        const doesImageUploadDone = await updateImagesItem(req, res);
        if (!doesImageUploadDone) {
            res.status(200).json(await db.updateItem(id, model, req));
        }
    } catch (error) {
        utils.handleError(res, error);
...
exports.createItem = async(req, res) => {
    try {
        const doesNameExists = await nameExists(
            req.body.name
        );
        if (!doesNameExists) {
