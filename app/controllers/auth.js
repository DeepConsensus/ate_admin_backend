...
const saveUserAccessAndReturnToken = async (req, user) => {
  return new Promise((resolve, reject) => {
    ...
    userAccess.save((err) => {
      ...
      Object.assign(userInfo, { password: undefined })
 ...
 */
const findUser = async (aadhar_number, contact_number) => {
  return new Promise((resolve, reject) => {
    User.findOne(
      {
        aadhar_number,
      ...
      // "password loginAttempts blockExpires name email role verified verification avatar id",
      (err, item) => {
...
const validateToken = async (authToken) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const response = await axios.post(goapi + 'token/validate', token)
      if (
        response &&
        response.data &&
...
exports.login = async (req, res) => {
  try {
    const data = req.body
    let user = await findUser(data.aadhar_number, data.contact_number)
    // var newobj = {
    //     firstName: data.firstName,
    //     lastName: data.lastName,
    //     photoUrl: data.photoUrl,
    // };
    // Object.assign(user._doc, newobj);
...
exports.register = async (req, res) => {
  try {
    req = req.body
    if (req.aadhar_number && req.contact_number) {
      const doesadharExists = await emailer.aadharExists(
        req.aadhar_number,
        req.contact_number
      )
      if (!doesadharExists) {
      ...
      res
        .status(200)
        .json({ isSuccess: false, message: 'Please check input values' })
    }
  } catch (error) {
    utils.handleError(res, error)
  }
  ...
  try {
    if (req.headers.authorization) {
      const tokenEncrypted = req.headers.authorization
        .replace('Bearer ', '')
        .trim()
      let isTokenValidated = await validateToken(tokenEncrypted)
      if (isTokenValidated.userType) {
        req.user = isTokenValidated
      }
      next()
    } else {
      next()
